...
This module provides functionality to collect and publish hardware information to a cloud service
via MQTT.
"""
...
class MQTTService:
    ...
    def _setup_mqtt_client(self):
        ...
        if self.broker_settings["username"] and self.broker_settings["password"]:
            self.client.username_pw_set(
                self.broker_settings["username"], self.broker_settings["password"]
            )
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            logger.info(
                f"Connected to MQTT broker at {self.broker_settings['host']}:{self.broker_settings['port']}"
            )
        else:
    ...
    def _publish_hardware_info(self):
        ...
        for topic_suffix, payload in topics.items():
            ...
            message_info = self.client.publish(
                topic=topic, payload=json.dumps(payload), qos=1, retain=False
            )
