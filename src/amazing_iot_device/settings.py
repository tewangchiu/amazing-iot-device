...
class MQTTSettingsForm(FlaskForm):
    ...
    mqtt_broker_port = IntegerField(
        "Broker Port", validators=[DataRequired(), NumberRange(min=1, max=65535)]
    )
    ...
    mqtt_publish_interval = IntegerField(
        "Publish Interval (seconds)",
        validators=[
            DataRequired(),
            NumberRange(min=5, message="Interval must be at least 5 seconds"),
        ],
    )
    ...
    def validate_mqtt_topic_prefix(self, field):
        ...
        if "/" not in field.data:
            raise ValidationError(
                "Topic prefix should include at least one hierarchy level (e.g., iot/device)"
            )
...
def mqtt():
    ...
    mqtt_settings = {
        s.key: s.value
        for s in Settings.query.filter(
            Settings.key.in_(
                [
                    "mqtt_enabled",
                    "mqtt_broker_host",
    ...
    mqtt_status = (
        mqtt_service.client and mqtt_service.client.is_connected()
    ...
    return render_template(
        ...
        last_published=None,
    )  # You could track last published time
...
def test_mqtt_connection():
    ...
    try:
        ...
        if connected[0]:
            ...
            result = {
                "success": False,
                "message": connection_error[0]
                or f"Timed out connecting to {broker_host}:{broker_port}",
            }

    except Exception as e:
        result = {"success": False, "message": f"Error connecting to MQTT broker: {str(e)}"}
